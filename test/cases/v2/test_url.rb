...
class TestUrl < Test::Unit::TestCase
  context "a new Bitly::V2::Url" do
    should "require a login and api_key" do
      assert_raise ArgumentError do Bitly::V2::Url.new end
      assert_raise ArgumentError do Bitly::V2::Url.new(login) end
      assert_raise ArgumentError do Bitly::V2::Url.new(nil, api_key) end
      assert_nothing_raised do
        Bitly::V2::Url.new(login, api_key)
        Bitly::V2::Url.new(login, api_key, :hash => '3j4ir4')
        Bitly::V2::Url.new(login, api_key, :short_url => 'http://bit.ly/3j4ir4')
        Bitly::V2::Url.new(login, api_key, :long_url => 'http://google.com/')
      end
    end
    ...
    context "shortening" do
      context "with a long url" do
        setup do
          ...
          @url = Bitly::V2::Url.new(login, api_key, :long_url => 'http://cnn.com/')
        end
      ...
      context "with no long url" do
        setup do
          @url = Bitly::V2::Url.new(login, api_key)
        end
      ...
      context "with a short url already" do
        setup do
          @url = Bitly::V2::Url.new(login, api_key, :short_url => 'http://bit.ly/31IqMl')
    ...
    context "expanding" do
      context "with a hash" do
        setup do
          ...
          @url = Bitly::V2::Url.new(login, api_key, :hash => '31IqMl')
        end
      ...
      context "with a long url already" do
        setup do
          @url = Bitly::V2::Url.new(login, api_key, :long_url => 'http://google.com')
          flexmock(@url).should_receive(:create_url).never
        end
    ...
    context "info" do
      context "with a hash" do
        setup do
          ...
          @url = Bitly::V2::Url.new(login, api_key, :hash => '3j4ir4')
        end
      ...
      context "with a short url" do
        setup do
          stub_get(/^http:\/\/api.bit.ly\/info\?.*hash=3j4ir4.*$/,"google_info.json")
          @url = Bitly::V2::Url.new(login, api_key, :short_url => 'http://bit.ly/3j4ir4')
        end
