...
class TestClient < Minitest::Test
  context "with a valid client" do
    ...
    context "validating another account credentials" do
      context "with valid credentials" do
        setup do
          stub_get(%r|http://api\.bitly\.com/v3/validate?.*x_login=correct.*|, "valid_user.json")
        end
      ...
      context "with invalid credentials" do
        setup do
          stub_get(%r|http://api\.bitly\.com/v3/validate?.*x_login=wrong.*|,"invalid_user.json")
        end
    ...
    context "checking a bitly pro domain" do
      context "with a bitly pro domain" do
        setup do
          stub_get(%r|http://api\.bitly\.com/v3/bitly_pro_domain?.*domain=nyti\.ms.*|, 'bitly_pro_domain.json')
        end
        should "return true" do
      ...
      context "with a non bitly pro domain" do
        setup do
          stub_get(%r|http://api\.bitly\.com/v3/bitly_pro_domain?.*domain=philnash\.co\.uk.*|, 'not_bitly_pro_domain.json')
        end
        should "return true" do
      ...
      context "with an invalid domain" do
        setup do
          stub_get(%r|http://api\.bitly\.com/v3/bitly_pro_domain?.*domain=philnash.*|, 'invalid_bitly_pro_domain.json')
        end
    ...
    context "shortening a url" do
      ...
      context "with extra options" do
        context "with the j.mp domain" do
          setup do
            stub_get( 'http://api.bitly.com/v3/shorten?longUrl=http%3A%2F%2Fbetaworks.com%2F&apiKey=test_key&login=test_account&domain=j.mp', 'betaworks_jmp.json'
            )
        ...
        context "with another domain" do
          setup do
            stub_get( 'http://api.bitly.com/v3/shorten?longUrl=http%3A%2F%2Fbetaworks.com%2F&apiKey=test_key&login=test_account&domain=nyti.ms', 'invalid_domain.json'
            )
          end
          should "raise an error" do
            assert_raises Bitly::Error do
        ...
        context "with another user details" do
          context "with correct details" do
            ...
            should "return a different hash" do
              assert @normal_url.user_hash != @other_user_url.user_hash
            end
          ...
          context "without an api key" do
            setup do
              stub_get( 'http://api.bitly.com/v3/shorten?longUrl=http%3A%2F%2Fbetaworks.com%2F&apiKey=test_key&login=test_account&x_login=other_account', 'invalid_x_api_key.json'
              )
            end
            should "raise an error" do
              assert_raises Bitly::Error do
    ...
    context "expanding a url" do
      context "a single url" do
        context "with a hash" do
          setup do
            @hash = '9uX1TE'
            stub_get(%r|http://api\.bitly\.com/v3/expand\?.*hash=9uX1TE.*|, '9uX1TE.json')
      ...
      context "multiple urls" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/expand?hash=9uX1TE&shortUrl=http%3A%2F%2Fbit.ly%2FcEFx9W&login=test_account&apiKey=test_key", 'multiple_urls.json')
    ...
    context "clicks for urls" do
      context "with multiple urls" do
        setup do
          @hash = '9uX1TE'
          @short_url = 'http://bit.ly/cEFx9W'
          stub_get("http://api.bitly.com/v3/clicks?hash=9uX1TE&shortUrl=http%3A%2F%2Fbit.ly%2FcEFx9W&login=test_account&apiKey=test_key", 'multiple_url_click.json')
    ...
    context "looking up" do
      context "a single url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/lookup?url=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", 'lookup_single_url.json')
      ...
      context "multiple urls" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/lookup?url=#{CGI.escape(@url1)}&url=#{CGI.escape(@url2)}&login=test_account&apiKey=test_key", 'lookup_multiple_url.json')
      ...
      context "a non existant url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/lookup?url=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", 'lookup_not_real_url.json')
          @lookup = @bitly.lookup(@url)
        end
        should 'return a missing url' do
    ...
    context "info for urls" do
      context "a single url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/info?shortUrl=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", "url_info.json")
          @info = @bitly.info(@url)
        end
        should "return a url object" do
      ...
      context "a single hash" do
        setup do
          @hash = "1YKMfY"
          stub_get("http://api.bitly.com/v3/info?hash=#{@hash}&login=test_account&apiKey=test_key", "url_info.json")
      ...
      context "multiple urls with urls and hashes" do
        setup do
          @url = 'http://bit.ly/1YKMfY'
          @hash = '9uX1TE'
          stub_get("http://api.bitly.com/v3/info?shortUrl=#{CGI.escape(@url)}&hash=#{@hash}&login=test_account&apiKey=test_key", "multiple_info.json")
      ...
      context "a nonexistant url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/info?shortUrl=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", 'not_found_info.json')
          @info = @bitly.info(@url)
        end
        should "return a missing url" do
    ...
    context "referrers for url" do
      context "a single url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/referrers?shortUrl=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", 'referrer_url.json')
      ...
      context "a single hash" do
        setup do
          @hash = 'djZ9g4'
          stub_get("http://api.bitly.com/v3/referrers?hash=#{CGI.escape(@hash)}&login=test_account&apiKey=test_key", 'referrer_url.json')
    ...
    context "countries for url" do
      context "a single url" do
        setup do
          @url = 'http://bit.ly/djZ9g4'
          stub_get("http://api.bitly.com/v3/countries?shortUrl=#{CGI.escape(@url)}&login=test_account&apiKey=test_key", 'country_url.json')
      ...
      context "a single hash" do
        setup do
          @hash = 'djZ9g4'
          stub_get("http://api.bitly.com/v3/countries?hash=#{CGI.escape(@hash)}&login=test_account&apiKey=test_key", 'country_hash.json')
    ...
    context "clicks by minute for urls" do
      context "with a single short url" do
        setup do
          ...
          stub_get("http://api.bitly.com/v3/clicks_by_minute?shortUrl=#{CGI.escape(@short_url)}&login=test_account&apiKey=test_key", 'clicks_by_minute1_url.json')
      ...
      context "with a single hash" do
        setup do
          @hash = '9DguyN'
          stub_get("http://api.bitly.com/v3/clicks_by_minute?hash=#{@hash}&login=test_account&apiKey=test_key", 'clicks_by_minute_hash.json')
      ...
      context "with multiple hashes" do
        setup do
          ...
          @hash2 = 'dvxi6W'
          @hashes = [@hash1, @hash2]
          stub_get("http://api.bitly.com/v3/clicks_by_minute?hash=#{@hash1}&hash=#{@hash2}&login=test_account&apiKey=test_key", 'clicks_by_minute_hashes.json')
    ...
    context "clicks by day for urls" do
      ...
      context "for multiple hashes" do
        setup do
          stub_get("http://api.bitly.com/v3/clicks_by_day?hash=9DguyN&hash=dvxi6W&login=test_account&apiKey=test_key", 'clicks_by_day.json')
        ...
        should "return a Time for the day" do
          ...
          assert_equal Time.at(1290488400), @urls[0].clicks_by_day[0].day_start
        end
      ...
      context "with optional days parameter" do
        should 'add days to url' do
          stub_get("http://api.bitly.com/v3/clicks_by_day?hash=9DguyN&hash=dvxi6W&login=test_account&apiKey=test_key&days=30", 'clicks_by_day.json')
  ...
  context "without valid credentials" do
    setup do
      ...
      stub_get(%r|http://api\.bitly\.com/v3/shorten?.*|, 'invalid_credentials.json')
    end
